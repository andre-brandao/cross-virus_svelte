<script lang="ts">
	...
	const emit_query_results = (results: any) => {
		const result_values = results.map(
			(res: any) => res.value,
		)
	...
	function highlightFeatures(
		...
		layerView.queryObjectIds(query).then(function (
			ids: any,
		) {
			if (highlightHandle) {
	...
	const queryStatsOnDrag = promiseUtils.debounce(
		(view: MapView, event: any) => {
			...
			const query_response = queryFeatureLayer(
				layerView,
				statsQuery,
			)
			...
			if (where) {
				console.log(
					'where clause for second dataset found',
					where,
				)
				...
				where_query_response = queryFeatureLayer(
					layerView,
					whereStatsQuery,
				)
			}
